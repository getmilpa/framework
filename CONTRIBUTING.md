# Contributing to Milpa Framework

Thanks for your interest in contributing! `milpa/framework` is the **starting-point template** for
a Milpa app — the thing people scaffold from with `composer create-project milpa/framework myapp`.
...
grows *from* this, not to the starting point itself (see "What this is NOT" in the README).
...
> out to work on this package against released dependencies.

## Guidelines

- **PHP >= 8.3**, with `declare(strict_types=1);` in every file.
- **Keep it minimal.** This package teaches the shape of a Milpa app; it is not the place to
